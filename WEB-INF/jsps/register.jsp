					...
					<div class="form-horizontal content-center">
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" name="userName"
									id="userName"><span id="userNameSpan"></span>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" name="password"
									id="password"><span id="passwordSpan"></span>
							</div>
						</div>
						...
						<div class="form-group">
							<label for="inputEmail3" class="col-sm-4 control-label">Re-Enter
								Password(前端验证)</label>
							<div class="col-sm-8">
								<input type="text" class="form-control" id="validatePassword">
								<span id="validatePasswordSpan"></span>
							</div>
						</div>
				...
				.blur(
						function() {
							...
							if (pattern.test($("#userName").val())) {
								$
										.ajax({
											url : "${pageContext.request.contextPath}/user/isExistUserName",
											...
											success : function(data) {
												if (data) {
													$("#userNameSpan")
															.html(
																	"<font color='red' size='3'>用户名已存在</font>");
												} else {
													$("#userNameSpan")
															.html(
																	"<font color='green' size='3'>用户名可用</font>");
												}
											}
										});
							} else {
								$("#userNameSpan")
										.html(
												"<font color='red' size='3'>用户名应由6~10位数字字母组成</font>");
							}
						});
		$("#password")
				.blur(
						function() {
							...
							if (pattern.test($("#password").val())) {
								...
								$("#passwordSpan")
										.html(
												"<font color='red' size='3'>密码应由6~10位数字字母组成</font>")
							}
						});
		$("#validatePassword").blur(
				function() {
					if ($("#validatePassword").val() != $("#password").val()) {
						$("#validatePasswordSpan").html(
								"<font color='red' size='3'>两次密码不一致</font>")
					}
				});
	</script>
